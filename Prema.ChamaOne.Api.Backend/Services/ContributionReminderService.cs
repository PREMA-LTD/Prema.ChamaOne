...
namespace Prema.ChamaOne.Api.Backend.Services
{
    public class ContributionReminderService : IHostedService, IDisposable
    {
        ...
        public ContributionReminderService(IServiceProvider serviceProvider, Logger logger, IMapper mapper)
        {
        ...
        private void CheckAndSendReminders(object state)
        {
            ...
            if (isDayOfDeadline(endOfMonth) ||
                isThreeDaysToDeadline(endOfMonth) ||
                isOneWeekToDeadline(endOfMonth) ||
                isTwoWeeksToDeadline(endOfMonth))
            {
        ...
        private string GetReminderMessage(DateTime endOfMonth, string memberName)
        {
            if(isDayOfDeadline(endOfMonth))
            {
                return $"Good morning {memberName}, today is the deadline for {endOfMonth.Month}. Kinldy clear before end of day as to avoid penalties. Thank you and have a great day."
            }
        }
